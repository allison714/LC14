...
%% Created 8/8/20 easyBar but with stats -RT
%% Allison + Garrett edits 8/12 for Regressive Slowing Battery 1
...
if pvals(3,1) < 0.05 || pvals(2,1) < 0.05
    ...
    s = clock;
    s = s(6);
    y=floor((s/100-floor(s/100))*100);
    %filename = sprintf('%s_RSB_epoch_%d.png',time,y);
    filename = sprintf('%s_RSB_epoch_%d_t.png',time,nn);
